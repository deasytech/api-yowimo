...
- ✅ New `WalletTransactionType::Reward` enum case (`app/Enums/WalletTransactionType.php`) — existing types (`TopUp`/`Purchase`/`Refund`/`Bonus`/`Adjustment`) didn't fit semantically; `WalletActivityBreakdownChart` iterates `::cases()` dynamically so no Filament changes were needed.
- ✅ `App\Listeners\GrantGameCompletionReward` (`ShouldQueue`, off `GameCompleted` only — mirrors the `Send*PushNotification` pattern's listener shape but calls `WalletService::credit()` instead of `Notification::send()`). Recipients: every distinct `user_id` with a `Turn` row on that `game_session_id` (not all party members — someone who joins after the game starts never gets a turn and isn't rewarded). Flat 25 tokens each, `description: 'Game completion reward'`, no `$reference` passed.
- ✅ Deliberately no `$reference` on the credit: `SendWalletCreditedPushNotification` (`app/Listeners/SendWalletCreditedPushNotification.php:25`) already skips the generic wallet notification for any transaction with `reference_type` set (written assuming that only happens for purchases). Referencing the `GameSession` would have silently suppressed the reward notification — confirmed with the user, chose no-reference to keep the existing notification path working over preserving an audit-trail FK. Zero changes to `SendWalletCreditedPushNotification` itself.
- ✅ Idempotency: `idempotencyKey: "game-completion-reward:{sessionId}:{userId}"` passed to `WalletService::credit()` guards against double-crediting on queue retry (first internally-generated idempotency key in the codebase — prior uses were all client-supplied `Idempotency-Key` headers passed through).
- ✅ Tests: `tests/Feature/Listeners/GrantGameCompletionRewardTest.php` — listener queued on `GameCompleted`, correct flat amount credited to every turn-taker, and a party member who joined after game start (never in `turn_order`, no `Turn` row) is confirmed to get no wallet at all.
- ✅ Full suite (278 tests) and Pint both pass with no regressions.
...
| **Game Engine (rounds/turns/timers)** | `game_sessions`/`rounds`/`turns` tables + `GameSessionService`; host-only start/next-turn, randomized turn order, host-configurable rounds, Truth/Dare card dealing with no-repeat-until-exhausted, auto-completion; 30s server-authoritative turn timer with AFK-skip (tracked per turn), crash-recovery sweep, and `RoundCompleted`/`GameCompleted` events; flat 25-token reward to every turn-taker on `GameCompleted`; Voting Engine + XP scoring (votes, MVP bonus, Challenge Completed XP) and Badges & Achievements (see Current Sprint above). | Daily streaks, combo multipliers, sponsor rewards, and leaderboards — the rest of the documented Reward/Scoring/Achievement Engine remains unbuilt and has no owning sprint in the current plan (see Current Priority). |
...
- Reward Engine beyond the flat game-completion grant and Voting/XP/Badges (daily streaks, combo multipliers, sponsor rewards, leaderboards) — none of that infrastructure exists (see `docs/architecture/08_GAME_ENGINE.md`'s Reward/Scoring/Achievement Engine sections); flat 25-token-per-turn-taker reward on `GameCompleted`, Voting Engine + XP scoring, and Badges & Achievements are now done (see Current Sprint above), the rest remains unscoped.
