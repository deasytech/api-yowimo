...
| **Game Engine (rounds/turns/timers)** | `game_sessions`/`rounds`/`turns` tables + `GameSessionService`; host-only start/next-turn, randomized turn order, host-configurable rounds, Truth/Dare card dealing with no-repeat-until-exhausted, auto-completion; 30s server-authoritative turn timer with AFK-skip (tracked per turn), crash-recovery sweep, and `RoundCompleted`/`GameCompleted` events; flat 25-token reward to every turn-taker on `GameCompleted` (see Current Sprint above). | Votes, scoring, XP/badges/streaks, and the rest of the documented Reward/Scoring/Achievement Engine — none of that is built; explicitly descoped from Sprint 7 by the user and has no owning sprint in the current plan (see Current Priority). |
...
- Reward Engine beyond the flat game-completion grant (voting, MVP, creativity, daily streaks, sponsors, achievements/badges, combo multipliers, leaderboards) — none of that infrastructure exists (see `docs/architecture/08_GAME_ENGINE.md`'s Reward/Scoring/Achievement Engine sections); flat 25-token-per-turn-taker reward on `GameCompleted` now done (see Current Sprint above), the rest remains unscoped.
