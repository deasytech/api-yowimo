...
| **AI Host ("Yowi")** | 🟡 Partial (Sprint 13, 2026-08-27) | `App\Services\AI\AIProvider` interface + `OpenAiProvider` (lean `Http`-facade call, no SDK package); `SendAiHostMessage` listener (queued, off `GameCompleted` only) broadcasts a playful AI-generated message via the new `AiHostMessageSent` event onto the existing `game-session.{id}` private channel; skip-silently-and-log on any OpenAI failure. No real OpenAI project/credentials configured in any environment yet (inert until `OPENAI_API_KEY` is set). The full "Yowi" persona (voice, moderation, translation, recommendations) and a `RoundCompleted` trigger remain unscheduled. |
