...
| **Marketplace (purchase flow, inventory, ownership)** | 🟡 Partial | Token bundle purchase (top-up) exists via `PurchaseService` with a manual/test `PaymentProvider` driver — no real payment gateway yet. Pack purchase/inventory/ownership (Sprint 3) still absent. |
