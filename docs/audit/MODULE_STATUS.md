...
| **Game Engine (rounds/turns/timers/votes/challenges)** | 🟡 Partial (Sprint 6 landed) | `game_sessions`/`rounds`/`turns` tables + `GameSessionService`; host-only `POST /parties/{id}/game/start` and `POST /game/{id}/next-turn` deal Truth/Dare cards and advance a poll-driven state machine (randomized turn order, host-configurable rounds, auto-completion). No timers, scoring, rewards, or votes yet — that's Sprint 7 per `08_GAME_ENGINE.md` (marked CRITICAL there). |
